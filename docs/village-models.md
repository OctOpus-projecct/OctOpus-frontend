...
## 캐릭터 체형 재작업
블록처럼 보인다는 피드백을 반영해 몸통·소매·팔·바지·부츠를 단면 곡선으로 정의하는 CharacterSurface 메시로 교체했다. 머리 비율을 키우고 손가락·연속된 미소·턱을 감싸는 수염과 몸에 맞춘 앞치마를 적용했다. 머리카락은 뿌리와 끝이 가늘고 중간이 부푸는 곡면이다. [캐릭터 확대 렌더](art/starting-village-characters.png)를 추가했다.

기존 Body/Head/ArmL/ArmR/LegL/LegR/Hand/HeldAxe 계층과 런타임 코드는 유지한다. 모델 생성·렌더, PlayMode 35개와 Windows 빌드를 검증했다. 별도 리뷰의 앞치마 포켓 매몰 지적은 곡률을 맞춰 해결했다. 마지막 머리카락 돌출 조정은 재생성·재렌더·빌드로 확인했다. 사용자 플레이 및 새 메시의 10명 성능 측정은 아직 미실행이다.
