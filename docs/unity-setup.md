...
이동 버튼은 기본 **Left**이며 화면의 **Left / Right / Middle**에서 변경한다. 설정은 PlayerPrefs에 저장되어 재실행 후에도 유지된다. 접속 패널 전체 영역 위 클릭과 게임 창에 포커스를 주는 클릭은 이동 요청을 보내지 않는다. 현재 [작은 생활 테스트 맵](test-map.md)은 24×24이며 중심 좌표 범위 ±11.5, Y=1, 속도 초당 4는 테스트용 값이다. 나무 5그루에서 벌목할 수 있다. 장애물 길찾기·카메라 조작은 후속 범위다.
...
## 검증 결과와 남은 범위
벌목 타격·스태미나 시제품은 [타격 확인 절차](striking.md)를 따른다. 아래 이동·나무 기록은 해당 버전의 검증 이력이다.
...
목재 지급·무게 제한의 실행/검증과 세션 초기화 범위는 [인벤토리 시제품](wood-inventory.md)을 따른다.

## 계정 버전 실행
공용 패키지 0.7.0, SHA `7289740a6b0e19e99bb16a1da0e1aaeb5dbad105`를 양쪽에서 사용한다. 정상 실행은 계정 인증을 요구한다. 계정 생성과 인증서/공개 지문 준비 후 접속한다. 기존 세션 회귀 스크립트만 명시적 익명 모드를 사용한다. 외부 회귀 서버에는 `-octopus-anonymous-regression`을 직접 지정해야 한다.
계정 운영은 백엔드 `docs/accounts.md`를 따른다. 목재는 계정별로 저장되며 이전 세션 초기화 설명은 익명 회귀 모드에만 해당한다.
