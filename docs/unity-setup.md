...
## 아직 확인하지 않은 범위
Windows 접속 화면은 2026-09-10 사용자가 두 창의 상태·ID·캡슐 표시와 접속 종료·재접속이 기대대로 동작한다고 확인했다. WSL Ubuntu 26.04 Linux 서버와 Windows 두 클라이언트의 접속·종료·재접속도 자동 검증을 통과했으며, 사용자가 실행 도우미와 실제 게임 창으로 동일한 기대 결과를 확인했다. Unity 공식 지원 Ubuntu 22.04·24.04 배포 환경과 10명 부하·원격 지연·AWS 검증은 남아 있다. GitHub 문서 CI와 Unity 실행 검증은 구분한다.
