param(
    [ValidateSet('Setup', 'Test', 'BuildWindows', 'BuildModels', 'RenderModels')]
...
if ($Task -eq 'RenderModels') { $arguments = $arguments.Replace('-nographics', '-force-d3d11') }
if ($Task -eq 'Test') {
    ...
    $entry = if ($Task -eq 'BuildModels') { 'VillageArtBuilder.Build' } elseif ($Task -eq 'RenderModels') { 'VillageArtBuilder.RenderPreviews' } else { "ProjectSetup.$method" }
    $arguments += " -quit -buildTarget $target -standaloneBuildSubtarget Player -executeMethod $entry"
}
